...
operators collapse to standard 4th roots of unity, and formalize four
Clifford algebras---$\mathrm{Cl}(1,1)$, $\mathrm{Cl}(3,0)$,
$\mathrm{Cl}(1,3)$---each independently verified. We propose a
...
This gap is a missed opportunity. Formal verification of
...
frameworks that diverge from standard mathematics. Mixed results show that formal
methods discriminate, and they reveal genuine mathematical structure
worth analyzing.
...
\item A \emph{case study} applying the protocol to Eric Dollard's versor
  algebra, producing over 200 formally verified declarations across 8 Lean~4 proof
  files with zero \texttt{sorry} gaps (\Cref{sec:case-study}).
\item A five-category \emph{taxonomy} for classifying claims from alternative
  ...
  (\Cref{sec:taxonomy}).
\item Four independently formalized \emph{Clifford algebras}---$\mathrm{Cl}(1,1)$,
  $\mathrm{Cl}(3,0)$, $\mathrm{Cl}(1,3)$---demonstrating that formal methods
  can suggest corrections, not merely judge (\Cref{sec:hierarchy}).
\end{enumerate}
...
and $jk = 1$ form a group under multiplication. These operators coincide with
the 4th roots of unity $\{1, i, -1, -i\}$ under complex multiplication.
...
Key products of the multiplication table are verified. The structure is trivially
...
In the standard Clifford-algebraic sense, versors
satisfy anticommutativity: in $\mathrm{Cl}(1,1)$,
...
The algebraic necessity result makes this ambiguity irrelevant:
rather than resolving it by fiat, we show that
...
Primary source analysis suggests a connection to Lagrangian circuit
theory: Dollard's $d\Psi/dt = I$ (current) and $d\Phi/dt = E$ (voltage)
correspond to the Lagrangian circuit variables where charge $q$ is the
generalized coordinate and flux $\lambda$ is the conjugate momentum.
The dimensional analysis is verified in Lean~4 (\texttt{circuit\_action.lean},
0~\texttt{sorry}); the Lagrangian identification itself is an interpretive
observation, not a formal result.
...
Mixed results across these categories are the \emph{expected} and
...
is merely standard mathematics with non-standard notation. The mixed outcome---some
verified, some disproved, some ambiguous---reveals genuine structure worth
analyzing, and shows that formal methods can distinguish valid from invalid
claims within a single framework.
...
Beyond classification, formal methods can suggest \emph{corrections}. We
formalize four Clifford algebras of increasing dimension, each independently
verified in Lean~4:

\begin{table}[ht]
\centering
\caption{Clifford algebras formalized in Lean~4 (independently defined, no connecting morphisms)}
\label{tab:hierarchy}
\begin{tabular}{@{}clcl@{}}
\toprule
...
At Level~2, $\mathrm{Cl}(1,1)$ provides what Dollard was reaching for: an
operator $e_1$ with $e_1^2 = +1$ that is distinct from $\pm 1$ by construction
(it occupies a different basis component). However,
...
in which Maxwell's equations take the compact form $\nabla F = J$. The Lean~4
formalization verifies the algebraic structure (signature, bivector decomposition,
Lorentz boost action) but not the differential equation itself, which requires
smooth manifold infrastructure not available in our formalization.

Dollard's \emph{instinct}---algebraic unification of electromagnetic
phenomena---was correct, while his \emph{execution}---a commutative group
with four elements---was far too restrictive. The Clifford algebras provide
the correct mathematical setting.
...
The sign error in the versor form (\Cref{sec:disproof}) could have been
noticed by informal calculation, but Lean provides machine-checked
certainty that the error is irreparable---a stronger conclusion than
informal reasoning alone supports.
...
The Clifford algebra formalizations (\Cref{sec:hierarchy}) suggest the
correct mathematical framework for Dollard's aspirations.
Showing not just what is wrong but what the author \emph{should} have
said is a constructive application of formal methods to
...
The Clifford algebra formalizations show that formal methods can constructively
suggest the ``correct'' version of an alternative framework's aspirations.

Formal methods separate what is mathematically checkable from what is not,
what is correct from what is wrong, and what is novel from what is standard.
We call for further case studies on other alternative mathematical frameworks
to validate and refine the protocol.
