...
foundation has been machine-verified in Lean~4 with the mathlib library: 43 proof files
containing approximately 1400 theorems with zero \texttt{sorry} gaps.  The $\mathrm{SO}(14)$
...
86\% of our theorems are signature-independent, holding for all real forms of $\so(14,\mathbb{C})$.
...
mathlib library~\cite{mathlib}.  Specifically, 43 Lean~4 proof files
containing approximately 1400 theorems have been compiled with zero
...
add up, the embeddings are valid Lie algebra homomorphisms (with the $A_4$
root system of $\su(5)$ explicitly verified), the anomalies cancel,
...
A signature audit of all 43 proof files~\cite{uft-github}
classifies 86\% as \emph{signature-independent} (properties of
...
Of 43 proof files containing ${\sim}1400$ theorems: 37 files (86\%)
...
\item \textbf{Signature audit}: classifying proofs by signature
  dependence (86\% independent, 7\% compact-only, 7\% needs-revision)
...
43 proof files.  One-loop renormalization group analysis yields coupling
