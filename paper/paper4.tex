...
\title{Three Fermion Generations from $E_8$: A Machine-Verified
...
\begin{abstract}
We present the first machine-verified resolution of the three-generation
problem in grand unified theories.  Using the Lean~4 theorem prover~\cite{demoura2021lean4}
with the mathlib library~\cite{mathlib}, we prove two results across 9 proof files
(over 575 verified declarations, zero \texttt{sorry} gaps).
\emph{Impossibility}: the semi-spinor $\Delta^+(14)$ of $\mathrm{Spin}(14)$
decomposes under $\mathrm{SO}(10) \times \mathrm{SO}(4)$ as
$(\mathbf{16},\mathbf{2}) \oplus (\overline{\mathbf{16}},\mathbf{2})$,
giving multiplicity~2 for the $\mathbf{16}$.  Since $3 \nmid 2$, no
intrinsic mechanism in any $\mathrm{SO}(10{+}2k)$ group can produce
exactly three generations\MV.
\emph{Resolution}: $E_8$ admits an $\mathrm{SU}(9)/\mathbb{Z}_3$
decomposition where $248 = 80 + 84 + \overline{84}$ and
$\Lambda^3(\mathbb{C}^9) = 84$ and its conjugate $\overline{84}$
together contain exactly three Standard Model
generations ($3 \times 16 = 48$ dimensions)\MV.  The number~3 arises as the dimension of the
fundamental of $\mathrm{SU}(3)_{\mathrm{family}} \subset
\mathrm{SU}(4) \subset \mathrm{SU}(9)$.
Both decompositions coexist inside $E_8$, connected through a
common $\mathrm{SU}(7) \times \mathrm{U}(1)$ subgroup.
...
We close this loop using interactive theorem proving.  In 9~Lean~4
proof files containing over 575 verified declarations with zero \texttt{sorry} gaps,
we establish:
\begin{enumerate}
  \item A \emph{universal} impossibility: no $\mathrm{SO}(10{+}2k)$
    gives three generations via spinor decomposition, because the
    family multiplicity $2^{k-1}$ is never~3\MV.
  \item A resolution via $E_8$: the $\mathrm{SU}(9)/\mathbb{Z}_3$
    maximal subgroup provides exactly three generations through the
    exterior algebra $\Lambda^3(\mathbb{C}^9)$\MV.
\end{enumerate}
To our knowledge, this is the first machine-verified three-generation
resolution for any GUT.  The methodology---Lean~4 with
mathlib---was developed for this
...
The obstruction is now arithmetic.  We prove
$3 \nmid 2$\MV\ (\texttt{three\_gen\_excluded}), which
immediately implies that exactly three copies of the $\mathbf{16}$
cannot be extracted from $\Delta^+(14)$ by any
...
under $\mathrm{SO}(14)$, because $\mathrm{SO}(14)$ acts
transitively on the Grassmannian $\mathrm{Gr}(10,14)$.
The orbit-stabilizer theorem gives $\dim \mathrm{Gr}(10,14) =
91 - 51 = 40$\MV\ (\texttt{orbit\_stabilizer\_check}).
...
The physical consequence is definitive: three fermion generations
\emph{cannot} arise from any mechanism intrinsic to $\mathrm{SO}(14)$
(or any $\mathrm{SO}(10{+}2k)$).  The generation mechanism must be
extrinsic.
...
eigenspaces (80, 84, $\overline{84}$)\MV.
...
The conjugate $\overline{84}$ decomposes analogously; crucially,
...
$84 - 48 = 36$, $48 + 36 = 84$, and $\dim(\mathrm{fund.\ SU}(3)) = 3$.
...
The complete argument is collected in the seven-part
\texttt{three\_generation\_theorem}\MV, which combines:
(a)~impossibility at $\mathrm{SO}(14)$ ($3 \nmid 2$),
(b)~embedding in $E_8$ ($91 + 1 + 28 + 64 + 64 = 248$),
(c)~$\mathrm{SU}(9)$ decomposition ($80 + 84 + 84 = 248$),
(d)~three generations ($3 \times 16 = 48 \leq 84$),
(e)~exotics ($84 - 48 = 36$),
(f)~compatibility ($49 + 21 + 21 = 91$), and
(g)~the origin of~3 ($3 \mid 3$ and $3 \nmid 2$).
...
The generation mechanism verified above is algebraic: it holds for
...
Wilson~\cite{wilson2024b} argues that $E_8(-24)$ is inherently massive
(containing $\mathrm{SU}(2,2)$ rather than the Poincar\'e group),
so the massless chirality assumption does not apply.
...
under $D_8 \supset D_7$ chirality (S$^+$ vs.\ S$^-$), giving a
chirality index $\chi = 35 - 21 = 14$\MV\
(\texttt{chirality\_index\_sector1}), while the total 248 remains
balanced ($64^+ = 64^-$)\MV, consistent with Distler--Garibaldi.
...
$J^2 = -\mathrm{Id}$\CO.
Crucially, $J$ does \emph{not} reduce to the chirality operator $\gamma_5$\CO:
...
This ``miraculous'' cancellation---previously an unexplained
feature of the Standard Model---becomes a consequence of $\sigma$
preserving the algebraic structure of $E_8$\CP.
...
\dim \mathrm{SO}(4) = 15 - 6 = 9$\MV, housing exactly
$\mathrm{SU}(3)$ ($\dim 8$) $+$ U(1) ($\dim 1$)\MV.
...
chain from impossibility through resolution.
...
We have proved, with machine verification, that three fermion
generations cannot arise from any orthogonal group
$\mathrm{SO}(10{+}2k)$ via spinor decomposition---a universal
impossibility---and that $E_8$ resolves this obstruction through
the $\mathrm{SU}(9)/\mathbb{Z}_3$ decomposition, where
$\Lambda^3(\mathbb{C}^9)$ naturally contains $3 \times 16 = 48$
dimensions of Standard Model matter.
...
sector (Definition~D\MV), compatible with the Distler--Garibaldi
no-go theorem.  The physical observable corresponding to massive
chirality remains open\OP.

The number~3 is not put in by hand.  It is the dimension of the
fundamental representation of $\mathrm{SU}(3)_\mathrm{family}$,
which emerges from the chain
$E_8 \supset \mathrm{SU}(9) \supset \mathrm{SU}(5) \times
\mathrm{SU}(4) \supset \mathrm{SU}(5) \times
\mathrm{SU}(3)_\mathrm{family} \times \mathrm{U}(1)$.

The complete argument---9 proof files, over 575 verified declarations, zero
\texttt{sorry} gaps---is publicly available~\cite{uft-github}.
