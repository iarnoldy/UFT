...
Using Lean~4 and mathlib, we extract formalizable assertions from such sources,
...
\item A \emph{case study} applying the protocol to Eric Dollard's versor
  algebra, producing 118 formally verified theorems across 8 Lean~4 proof files
  with zero \texttt{sorry} gaps (\Cref{sec:case-study}).
...
Fleuriot and Paulson~\cite{fleuriot2001newton} formalized Newton's geometric
...
for analyzing polyphase power systems and transmission
...
\emph{Advanced Versor Algebra II-23}. We extracted 17 formalizable claims
from these sources.
...
noncomputable def h : ℂ := -1
noncomputable def k : ℂ := h * j

theorem j_fourth : j ^ 4 = 1 := by ...
theorem h_squared : h ^ 2 = 1 := by ...
theorem jk_identity : j * k = 1 := by ...
\end{lstlisting}
...
\begin{align*}
  \text{Versor form:} \quad & h(XB + RG) + j(XG - RB) = -(XB + RG) + j(XG - RB) \\
  \text{Telegraph product:} \quad & (RG + XB) + j(XG - RB)
\end{align*}
...
\begin{lstlisting}[caption={Versor form disproof (excerpt from \texttt{telegraph\_equation.lean})}]
...
theorem algebraic_necessity_general {F : Type*}
    [Field F] [CharZero F]
    (j h k : F) (hj2 : j^2 = -1)
    (hhj : h * j = k) (hjk : j * k = 1) :
    h = -1 := by ...
...
(\Cref{sec:disproof}) makes the notation irrelevant. Whatever
...
$\lambda$ is the conjugate momentum~\cite{fortescue1918}. The dimensional
...
intend. This is distinct from \textsc{Disproved} (which requires the claim to
be explicitly stated and false) and from \textsc{Verified} (which implies the
author's claim is correct). Algebraic necessity reveals that the framework is
more constrained than its author believed---a finding that is neither
verification nor refutation but structural analysis.
...
At Level~4, $\mathrm{Cl}(1,3)$ is the spacetime algebra where Maxwell's four
equations collapse to one: $\nabla F = J$. This is the genuine unified field
...
translation ambiguities that are inherent to the domain. We formalize algebraic
and structural claims only; we make no claims about the physics, engineering
utility, or broader validity of Dollard's work.
...
ambiguous, and unfalsifiable claims---that reveal genuine mathematical
structure. The five-category taxonomy provides a systematic classification
scheme. The Clifford algebra hierarchy shows that formal methods can
constructively suggest the ``correct'' version of an alternative framework's
aspirations.
...
All Lean~4 proof files, source materials, and analysis documents are available
at \url{https://github.com/iarnoldy/UFT}.
...
abstract \texttt{CliffordAlgebra} via isomorphism proofs.
...
Fleuriot, J., Paulson, L.: Mechanizing Nonstandard Real Analysis.
