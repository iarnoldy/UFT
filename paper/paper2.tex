...
% NOTE: Switch to svjour3 before AACA submission. Using article for drafting.
...
% Acknowledgements environment (svjour3 defines this; article does not)
...
\texttt{sorry} gaps. The proof files are publicly available
\cite{uft-github}.
...
More finely, it admits a $\ZZ$-filtration by \emph{grade}:
...
The Fortescue analogy is now concrete:
\begin{itemize}[leftmargin=*]
...
$c_2(\text{fund}) = 13/2$, verified in Lean~4 via the arithmetic
identity $14 \times 12 / (2 \times 14) = 13/2$.
...
\item The Coxeter number $h$ enters Lie theory through the formula
  $|W| = r! \prod_{i=1}^r (m_i + 1) / r!$ and the dual Coxeter number
...
$\leq n$). For $p = q = 2$: the product lies in grades $0$ and $2$.

The commutator $[A, B] = AB - BA$ of two bivectors is therefore a
pure bivector (grade 2), since the grade-0 parts cancel:
if $AB$ has grade-0 part $s$ and grade-2 part $t$, then $BA$ has
grade-0 part $s$ (symmetric) and grade-2 part $-t + 2s'$ (where $s'$
accounts for the scalar). The antisymmetric combination kills grade~0,
confirming \Cref{prop:grade2-lie}.
...
\begin{proof}
The even subalgebra $\Cl^+(n,0) = \bigoplus_{k \text{ even}} \Cl^k$
is a subalgebra (closed under multiplication), and any product of
grade-2 elements lies in $\Cl^+$. Hence $\hat{H}$ maps even grades
...
The full codebase, including all 37 Lean~4 proof files with
approximately 900 theorems, is available at \cite{uft-github}.
...
power systems analogy: the Cartan subalgebra is the ``zero sequence''
...
The analogy is particularly vivid for $\su(2)$, where the three
...
in Lean~3/mathlib, providing the mathematical infrastructure that
underlies our concrete computations. Tooby-Smith's PhysLean/HepLean
