...
\begin{abstract}
$\mathrm{SO}(14)$ is the smallest orthogonal group that contains both the Standard Model
gauge group $\mathrm{SO}(10)$ and a gravity sector $\mathrm{SO}(4)$.  Its 91 generators
split as $45 + 6 + 40$: gauge, gravity, and the interactions between them.
We build the algebraic scaffold for this theory and check every step with the
Lean~4 theorem prover---88 proof files, over 2,900 verified declarations, zero
\texttt{sorry} gaps.

What the computer checks: five certified Lie algebra homomorphisms composing
$\su(5) \hookrightarrow \so(10) \hookrightarrow \so(14) \hookrightarrow \so(14)_M
\hookrightarrow \so(16)$, the Jacobi identity for nine Lie algebras including the
248-dimensional $E_8$, anomaly trace identities, and subalgebra closures for both
$\mathrm{SO}(16)$ and $\mathrm{SU}(9)$ inside $E_8$.
What the computer does not check: whether $\mathrm{SO}(14)$ describes nature.

The breaking chain runs $\mathrm{SO}(14) \to \mathrm{SO}(10) \times \mathrm{SO}(4)
\to \cdots \to \mathrm{U}(1)_\text{EM}$ via a symmetric traceless $\mathbf{104}$ Higgs.
One-loop running [CO] gives coupling unification at $10^{16.98}\GeV$ with a 0.88\%
miss, proton lifetime $10^{38.9}$ years (safe), and asymptotic freedom above the SM scale.
Three open problems: three generations, Distler ghosts, and the mass gap.
77\% of the algebraic results hold for all real forms of $\so(14,\mathbb{C})$.
No grand unified theory has previously received machine-checked algebraic scaffolding.
\end{abstract}
...
The Standard Model works. Three forces, 19 free parameters, no gravity.
Grand unified theories try to fix this by embedding
$\mathrm{SU}(3) \times \mathrm{SU}(2) \times \mathrm{U}(1)$ in a bigger
group~\cite{georgi1974,fritzsch1975,georgi1975,weinberg1967,glashow1961,salam1968}.
$\mathrm{SO}(10)$ is the workhorse: one 16-dimensional spinor holds a
complete generation of fermions~\cite{fritzsch1975}; see
Baez--Huerta~\cite{baez2010} for a modern review.  But $\mathrm{SO}(10)$
says nothing about gravity.

$\mathrm{SO}(14)$ is the smallest orthogonal group that fits both $\mathrm{SO}(10)$
and the Lorentz group $\mathrm{SO}(1,3)$~\cite{nesti2007,nesti2010,krasnov2018}.
Nesti, Percacci, and Krasnov developed the physics.
What was missing: a check that the algebra actually works.

That is what this paper provides.  We built the algebraic scaffold in
Lean~4~\cite{demoura2021lean4} with mathlib~\cite{mathlib}: 88 proof files,
over 2,900 verified declarations, zero \texttt{sorry} gaps.
The proofs compile or they don't.  Ours compile.

The scaffold is not just arithmetic.  Five certified Lie algebra
homomorphisms compose into a chain from $\su(5)$ through $\so(16)$.
Nine Lie algebras carry verified Jacobi identities, including the
248-dimensional $E_8$ (3.78 billion evaluations via \texttt{native\_decide}).
Both $\mathrm{SO}(16)$ and $\mathrm{SU}(9)$ are verified as genuine
subalgebras of $E_8$ via bracket closure.
The anomaly trace identity $\Tr(A\{B,C\}) = 0$ is proved for antisymmetric
matrices over any ring.  These are algebraic theorems, not dimensional
coincidences.

But there is also arithmetic.  Dimension counts, Goldstone boson numbers,
beta-coefficient inputs---all checked by \texttt{norm\_num}.
Both tiers use the same verification infrastructure---Lean~4 and
mathlib---as the Flyspeck project~\cite{hales2017formal} and the
liquid tensor experiment~\cite{commelin2022liquid}, though the
mathematical depth of our proofs (primarily arithmetic and
finite-dimensional linear algebra) is not comparable to those efforts.

No grand unified theory has previously been machine-checked.
From the outset, we distinguish what is proved from what is proposed:

\begin{itemize}
\item \textbf{Machine-verified} [MV]: Two tiers.  \emph{Structural} [MV-structural]: five certified Lie algebra homomorphisms ($\su(5) \hookrightarrow \so(10) \hookrightarrow \so(14) \to \so(14)_M \hookrightarrow \so(16)$, plus $\so(4) \hookrightarrow \so(14)$), the Jacobi identity for nine Lie algebras (including the 248-dimensional $E_8$ via 3.78 billion evaluations), the $A_4$ Cartan matrix of $\su(5) \subset \so(10)$ (48 bracket relations), centralizer closure, gauge-gravity commutativity, anomaly trace identities, and $\so(16) \subset E_8$ subalgebra closure.  \emph{Arithmetic} [MV-arithmetic]: dimensional consistency checks for anomaly prerequisites, Goldstone boson counts, beta-coefficient inputs, and representation dimensions.  Both tiers hold for the complex Lie algebra $\so(14,\mathbb{C})$ and all its real forms including $\mathrm{SO}(3,11)$.
...
It proves that the algebra is consistent.  Whether nature uses this
algebra is a question for experiment.

Section~\ref{sec:foundation} covers the machine-verified foundation.
Section~\ref{sec:model} builds the $\mathrm{SO}(14)$ model.
Section~\ref{sec:rg} runs the couplings.
Section~\ref{sec:pheno} extracts predictions.
Section~\ref{sec:discussion} states the open problems honestly.
Section~\ref{sec:conclusion} says what we proved and what we didn't.
...
The codebase compiles as a single Lean~4 project (over 3,400 build jobs,
...
$\Tr(A\{B,C\}) = 0$ for antisymmetric %[MV]
  & \texttt{anomaly\_trace} & \texttt{trace\_antisymm\_anticommutator} & Algebraic anomaly identity \\
$14 \geq 7$ anomaly threshold %[MV]
  & \texttt{so14\_anomalies} & \texttt{anomaly\_checklist} & Arithmetic prerequisite (\texttt{norm\_num}) \\
...
$[\so(4), \so(10)] = 0$ in $\so(14)$ %[MV]
  & \texttt{unification\_gravity} & \texttt{gravity\_gauge\_commute} & Disjoint index sets \\
...
Jacobi identity for $\so(14)$ (91 gens) %[MV]
  & \texttt{so14\_grand} & \texttt{jacobi} & Cubic polynomial, \texttt{ring} \\
$E_8$ Jacobi (248 dims, 3.78B evals) %[MV]
  & \texttt{e8\_sparse\_jac\_*} & \texttt{e8\_jacobi\_*} & Sparse \texttt{native\_decide} \\
$\su(5) \hookrightarrow \so(10)$ LieHom %[MV]
  & \texttt{su5c\_so10\_liehom} & \texttt{su5c\_embed} & 45 bracket equations \\
$\so(10) \hookrightarrow \so(14)$ LieHom %[MV]
  & \texttt{so10\_so14\_liehom} & \texttt{so10\_embed} & 91 bracket equations \\
$\so(14) \to \so(14)_M$ type bridge %[MV]
  & \texttt{so14\_to\_matrix} & \texttt{so14\_bridge} & 196 matrix entries \\
$\so(14)_M \hookrightarrow \so(16)_M$ LieHom %[MV]
  & \texttt{so16\_matrix} & \texttt{so14m\_embed\_block} & Block embedding \\
$\mathrm{SU}(9) \subset E_8$ subalgebra closure %[MV]
  & \texttt{e8\_su9\_subalgebra} & \texttt{su9\_subalgebra\_e8} & 80 gens, \texttt{native\_decide} \\
...
\end{equation}
where $\so(6) \cong \su(4)$ (Pati-Salam~\cite{patisalam1974}), $\so(10)$ (Georgi-Glashow-Fritzsch),
and $\so(14)$ (this work).  Five certified Lie algebra homomorphisms compose into a chain:
$\su(5) \xrightarrow{\text{LieHom}} \so(10) \xrightarrow{\text{LieHom}} \so(14) \xrightarrow{\text{bridge}} \so(14)_M \xrightarrow{\text{LieHom}} \so(16)$,
where $\so(14)_M$ denotes the mathlib matrix representation (the ``type bridge'' connecting the flat algebraic construction to mathlib's \texttt{LieSubalgebra} infrastructure).  Additionally, $\so(4) \xrightarrow{\text{LieHom}} \so(14)$ embeds the gravity sector.  The $E_8$ Lie algebra (248 dimensions) is independently verified via sparse Jacobi identity with \texttt{native\_decide}, and $\so(16) \subset E_8$ is verified as a subalgebra closure.
...
A signature audit of 47 proof files~\cite{uft-github}
classifies 77\% (36 files) as \emph{signature-independent} (properties of
$\so(14,\mathbb{C})$, holding for all real forms), 2\% (1 file) as
\emph{compact-only} (the Pauli algebra $\Cl(3,0)$), and 21\% (10 files) as
\emph{signature-sensitive} (results whose physical interpretation
depends on the choice of real form). %[CO]
...
via an adjoint $\mathbf{45}$ VEV\SP\ (dimension $\dim\,\mathbf{45} = 45$\MV);
$\mathrm{SU}(5) \to \mathrm{SU}(3) \times \mathrm{SU}(2) \times \mathrm{U}(1)_Y$
via an adjoint $\mathbf{24}$ VEV\SP\ (dimension $\dim\,\mathbf{24} = 24$\MV);
electroweak breaking via a fundamental Higgs doublet\SP\ (dimension $\dim\,\mathbf{4} = 4$\MV).
...
Of the 47 proof files audited for signature dependence: 36 files (77\%)
are signature-independent, 1 file (2\%) is compact-only (the Pauli
algebra $\Cl(3,0)$), and 10 files (21\%) are signature-sensitive
(physical interpretation depends on the choice of real form)\CO.
The precise classification is documented in Ref.~\cite{uft-github}.
...
This is a consistency check: three free scales are adjusted to minimize
three coupling mismatches, so the small residual confirms internal
consistency rather than constituting a prediction of unification\CO.
...
two-loop corrections, or split multiplet spectra\CO.
...
\paragraph{Scale inaccessibility.}
The phenomenological predictions of $\mathrm{SO}(14)$ in the desert
regime---coupling unification, proton decay, asymptotic freedom---are
identical to those of minimal $\mathrm{SO}(10)$.  The
$\mathrm{SO}(14)$-specific signatures (mixed gauge bosons, SO(4)
sector physics) appear at $M_1 \sim 10^{19}\GeV$ and are currently
inaccessible by many orders of magnitude.  The value of
$\mathrm{SO}(14)$ relative to $\mathrm{SO}(10)$ is therefore
structural---gravity inclusion and $E_8$ embedding---rather than
phenomenological.  No currently planned experiment can distinguish
$\mathrm{SO}(14)$ from $\mathrm{SO}(10)$\CO.
...
E$_8$~\cite{lisi2007} & 248 & $\mathbf{248}$ & --- & Partial$^\dagger$ & Yes & claimed & not computed \\
SO(18)~\cite{wilczek1982} & 153 & $\mathbf{256}$ & No & No & No & 8+8 mirror & killed \\
\bottomrule
\multicolumn{8}{@{}p{0.95\textwidth}}{\footnotesize $^\dagger$This work provides the first machine-verified $E_8$ Lie algebra (248 dims, Jacobi identity); Lisi's original proposal~\cite{lisi2007} contained no formal verification.}
\end{tabular}
\end{table*}
...
\item \textbf{Gravity inclusion}: the $\mathrm{SO}(3,1)$ Lorentz group
  is embedded as a subgroup, with $[\so(10), \so(4)] = 0$\MV\ (proven for compact indices; the result is signature-independent and holds for $\so(1,3)$ in the Lorentzian real form).
...
mechanisms at once\MV.  The generation mechanism must therefore be
...
are verified across 11 Lean~4 files with zero \texttt{sorry} gaps.
$E_8$ itself is now machine-verified as a Lie algebra
(248 dimensions, 7,752 nonzero brackets, Jacobi identity verified
on 3.78 billion evaluations via \texttt{native\_decide}).
Both maximal subgroups---$\mathrm{SO}(16)$ (120 generators) and
$\mathrm{SU}(9)$ (80 generators)---are verified as genuine Lie
subalgebras of $E_8$ via bracket closure\MV, and
each $J$ eigenspace of the $\mathbb{Z}_3$ automorphism is
independently anomaly-free\MV.
The $\mathrm{SO}(14)$-intrinsic generation problem remains open\OP;
the $E_8$-level resolution is algebraically verified\MV\ but involves
unresolved physics (chirality definition, signature dependence).
...
Wilson~\cite{wilson2022} argues that for theories with explicitly
...
Interactive theorem provers have not previously been applied to GUT physics.
...
\item \textbf{Signature audit}: classifying proofs by signature
  dependence (77\% independent, 2\% compact-only, 21\% signature-sensitive)
...
\item \textbf{$\mathrm{E}_8$ generation mechanism} (partially complete):
  $E_8$ is now machine-verified as a Lie algebra (248 dimensions,
  Jacobi on 3.78 billion evaluations\MV), $\mathrm{SO}(16) \subset E_8$
  is verified as a subalgebra closure\MV, and the three-generation
  mechanism via $\mathrm{SU}(9)/\mathbb{Z}_3$ is dimensionally
  verified across 10 proof files~\cite{arnoldy2026prl}.  Each
  $J$ eigenspace of the $\mathbb{Z}_3$ automorphism is independently
  anomaly-free\MV.
  Remaining: connecting the $\mathrm{Spin}(3,11)$ real form to the
  verified compact scaffold, and resolving the chirality
  definition for massive fermions in $E_{8(-24)}$.
...
Here is what we built.  88 Lean~4 proof files. Over 2,900 verified
declarations. Zero \texttt{sorry} gaps.  Five Lie algebra homomorphisms
chain $\su(5)$ through $\so(16)$.  $E_8$ is verified as a Lie algebra.
Both $\mathrm{SO}(16)$ and $\mathrm{SU}(9)$ close as subalgebras inside it.
The proofs compile.  Anyone can check.

Here is what the proofs say about $\mathrm{SO}(14)$.  The algebra is
consistent: 91 generators split $45 + 6 + 40$, the embeddings preserve
brackets, the anomaly traces vanish, the Goldstone counts work out.
One-loop running [CO] gives $M_\text{GUT} = 10^{16.98}\GeV$, proton
lifetime $10^{38.9}$ years, and asymptotic freedom above the SM scale.

Here is what the proofs do not say.  They do not say $\mathrm{SO}(14)$
describes nature.  Three open problems remain: why three generations
(every GUT has this problem), ghost states in the non-compact form
(every gravity-gauge unification has this problem), and the mass gap
(that is the Millennium Prize problem).

The algebra can't be wrong.  The physics might be.  We think that
distinction is worth making precisely, and that machine verification
should be standard practice for future GUT proposals.
...
\bibitem{wilson2022}
R.~A.~Wilson,
``Chirality in an $\mathrm{E}_8$ model of elementary particles,''
arXiv:2210.06029 (2022).
