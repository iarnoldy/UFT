...
% Fallback definitions for svjour3 commands (article class lacks these)
...
\providecommand{\keywords}[1]{%
  \begingroup\renewcommand{\and}{$\cdot$}%
...
\institute{  % svjour3 only; ignored under article class
  Independent Researcher \\
  \email{ian@arnoldy.me}
}

% \date{Received: date / Accepted: date}  % uncomment for svjour3
\date{\today}
...
pure bivector (grade 2), since the grade-0 part of the Clifford
product is symmetric: $\langle AB \rangle_0 = \langle BA \rangle_0$
(it equals the scalar product $A \cdot B$). Hence the antisymmetric
combination $AB - BA$ has zero grade-0 component, leaving only
grade~2, which confirms \Cref{prop:grade2-lie}.
...
The full codebase, including all 35 Lean~4 proof files with
approximately 878 theorems, is available at \cite{uft-github}.
