...
%   Paper 2: "Character Decomposition from Fortescue to Cartan-Weyl" (AACA, in prep)
%
% Claim tags:
...
\begin{abstract}
We present a candidate unified field theory based on $\mathrm{SO}(14)$, whose algebraic
foundation has been machine-verified in Lean~4 with the mathlib library: 37 proof files
containing approximately 980 theorems with zero \texttt{sorry} gaps.  The $\mathrm{SO}(14)$
...
analysis yields coupling unification at $M_\text{GUT} = 10^{16.98}\GeV$ with
$\alpha_\text{GUT}^{-1} = 47.03$ (0.88\% miss with threshold corrections from 178
scalar components), a proton lifetime of $\tau_p \approx 10^{38.9}$~years (safely
above the Super-Kamiokande bound of $10^{34.4}$~years), and asymptotic freedom at
all scales above the Standard Model.  We state open problems honestly: the three-generation
...
83\% of our theorems are signature-independent, holding for all real forms of $\so(14,\mathbb{C})$.
To our knowledge, this is the first grand unified theory whose algebraic foundation
has been formally verified by an interactive theorem prover.
\end{abstract}
...
a spectacularly successful description of the strong, weak, and electromagnetic
...
a distinctive feature: the algebraic foundation has been machine-verified using
the Lean~4 interactive theorem prover~\cite{demoura2021lean4} and the
mathlib library~\cite{mathlib}.  Specifically, 37 Lean~4 proof files
containing approximately 980 theorems have been compiled with zero
\texttt{sorry} gaps (unproved assertions).  This means that the algebraic
structure---dimensions, embeddings, anomaly cancellation, representation
decompositions, breaking chain, beta-function coefficients, and
more---has been checked by computer with the same level of rigor as the
proofs in the Flyspeck project~\cite{hales2017formal} or the liquid
tensor experiment~\cite{commelin2022liquid}.

To our knowledge, this is the first grand unified theory to receive
machine verification of its algebraic structure.  We emphasize from the
outset the distinction between what is proved and what is proposed:

\begin{itemize}
\item \textbf{Machine-verified} [MV]: Algebraic identities, dimensions,
  embeddings, anomaly cancellation, branching rules, and beta-function
  coefficients.  These are properties of the complex Lie algebra
  $\so(14,\mathbb{C})$ and its representations, holding for all real
  forms including $\mathrm{SO}(3,11)$.
...
It proves that the algebraic scaffold is internally consistent: the dimensions
add up, the embeddings are valid Lie algebra homomorphisms, the anomalies cancel,
and the breaking chain produces the correct Standard Model gauge group with
the correct particle content.  Whether this scaffold corresponds to physical
...
All algebraic results are formalized in Lean~4~\cite{demoura2021lean4},
...
The codebase compiles as a single Lean~4 project (3,296 build jobs,
zero errors, zero \texttt{sorry} gaps) and is publicly
available~\cite{uft-github}.
...
\caption{Principal machine-verified results.  All proofs compile with
  zero \texttt{sorry} gaps in Lean~4 with mathlib.}
\label{tab:mv}
\small
\begin{tabular}{@{}llll@{}}
\toprule
...
$91 = 45 + 6 + 40$ %[MV]
  & \texttt{so14\_unification} & \texttt{unification\_decomposition} & Gauge + gravity + mixed \\
...
All 6 anomaly conditions %[MV]
  & \texttt{so14\_anomalies} & \texttt{anomaly\_checklist} & SO(14) anomaly freedom \\
$Q = T_3 + Y/2$ preserves VEV %[MV]
  & \texttt{symmetry\_breaking} & \texttt{charge\_preserves\_vev} & Photon masslessness \\
$16 = 1 + 10 + \overline{5}$ %[MV]
  & \texttt{spinor\_matter} & \texttt{spinor\_weight\_decomposition} & One generation per spinor \\
Higgs: sym.\ traceless $\mathbf{104}$ %[MV]
  & \texttt{so14\_breaking\_chain} & \texttt{so14\_sym\_traceless\_dim} & Corrected Higgs rep \\
$75$ massive $+ 16$ massless $= 91$ %[MV]
  & \texttt{so14\_breaking\_chain} & \texttt{gauge\_boson\_conservation} & Mass spectrum accounting \\
$[\so(1,3), \so(10)] = 0$ %[MV]
  & \texttt{unification\_gravity} & \texttt{commutator\_vanishes} & Gravity-gauge decoupling \\
SM beta coefficients %[MV]
  & \texttt{rg\_running} & \texttt{beta1\_gut\_normalized} & $b_1, b_2, b_3$ from reps \\
Jacobi identity for $\so(10)$ %[MV]
  & \texttt{so10\_grand} & \texttt{jacobi\_cyclic} & Lie algebra axiom \\
$\nabla \cdot B = 0$ from Bianchi %[MV]
  & \texttt{bianchi\_identity} & \texttt{gauss\_law\_magnetism} & Maxwell from algebra \\
\bottomrule
\end{tabular}
\end{table*}
...
A signature audit of all 37 proof files~\cite{uft-github}
classifies 83\% as \emph{signature-independent} (properties of
$\so(14,\mathbb{C})$, holding for all real forms), 8\% as
\emph{compact-only} (energy positivity results that fail for
$\mathrm{SO}(3,11)$ above the breaking scale), and 8\% as
...
where:
\begin{itemize}
\item The 45 generators of $\so(10) \cong \su(5) \oplus \cdots$
  contain the Standard Model gauge bosons plus leptoquarks.
\item The 6 generators of $\so(4) \cong \su(2) \times \su(2)$
  are identified with the Lorentz group in the non-compact form
  $\so(3,1)$.\CP
\item The 40 generators in the $(\mathbf{10}, \mathbf{4})$
  bifundamental representation mix gauge and gravitational
  indices.\MV
\end{itemize}

The crucial algebraic fact is that the gauge and gravity subalgebras
commute\MV:
...
The $\mathrm{SO}(14)$ algebra is anomaly-free.  All six standard
anomaly conditions---including the quartic $d$-symbol trace
$\binom{93}{3} = 129{,}766$---are machine-verified.\MV{}  Anomaly
cancellation is a property of the complex Lie algebra and holds
...
Model proceeds in five steps\MV\CO:
...
$\mathrm{SO}(2n_1) \times \mathrm{SO}(2n_2)$~\cite{li1974,michel1973}.
...
\end{equation}
The $(\mathbf{10}, \mathbf{4}) = 40$ components are eaten as Goldstone
bosons by the 40 broken gauge bosons.\MV{}  The remaining 64 components
...
bosons.  The remaining $91 - 75 = 16$ massless bosons are: 8 gluons,
1 photon, 6 SO(4) generators (gravity sector), and 1 extra
$\mathrm{U}(1)_\chi$.\MV
...
\caption{Symmetry-breaking chain.  ``Broken'' = number of broken generators
  ...
  All dimension counts are machine-verified [MV].}
\label{tab:breaking}
\small
\begin{tabular}{@{}llccc@{}}
\toprule
...
the right-handed neutrino)\MV.\SP{}  The $\overline{\mathbf{16}}$ is
...
Six candidate mechanisms exist for generating three families within
$\mathrm{SO}(14)$:
\begin{enumerate}
\item Orbifold family unification in $\mathrm{SO}(2N)$ gauge
  theory~\cite{kawamura2010}, which has been demonstrated for the
  $\mathrm{SO}(2N)$ series and can yield three families with
  specific boundary conditions.
\item The Clifford algebra $S_3$ symmetry
  mechanism~\cite{gresnigt2024,gresnigt2026}, which produces exactly
  three copies from the sedenion structure of $\Cl(8) \subset \Cl(14)$
  via a unique splitting into three complex octonion subalgebras.
\item Krasnov's octonionic framework~\cite{krasnov2022}, in which
  the 64-dimensional semi-spinor of $\mathrm{Spin}(11,3)$ is
  identified with $\mathbb{O} \otimes \mathbb{O}'$ (octonions times
  split octonions), with a potential quaternionic triple mechanism
  for three generations.
\item Calabi-Yau compactification via the $\mathrm{SO}(14) \times
  \mathrm{U}(1) \subset \mathrm{E}_8$ embedding, where the Euler
  characteristic $\chi = \pm 6$ gives three generations~\cite{candelas1985}.
\item Discrete flavor symmetry $A_4 \subset \mathrm{SO}(4)$, exploiting
  the binary tetrahedral subgroup of $\mathrm{SU}(2)$~\cite{altarelli2005}.
\item The BenTov-Zee mechanism~\cite{bentov2016}, which revives
  $\mathrm{SO}(2N)$ family unification via topological
  superconductor technology.
\end{enumerate}
The most algebraically promising mechanism is (2), the $\Cl(8)$ $S_3$
symmetry, which uniquely produces the number three from algebraic
structure.  Its extension from $\Cl(8)$ to the $\mathbf{64}$-spinor of
$\Cl(14)$ is a well-defined open problem\OP.
...
the same open question while offering additional algebraic
structure ($\mathrm{SO}(4)$ factor, octonionic spinor,
$\Cl(14) \supset \Cl(8)$ chain) that is not available to
$\mathrm{SO}(10)$.
...
equivalently $\mathrm{Spin}(3,11)$), whose maximal compact subgroup
$\mathrm{SO}(3) \times \mathrm{SO}(11) \supset \mathrm{SO}(3,1)
\times \mathrm{SO}(10)$ contains both the Lorentz group and the
GUT gauge group\CP.  Following Nesti and
...
signatures: $\Cl(14,0) \cong M(128, \mathbb{R})$ has quaternionic
...
Of 37 proof files containing ${\sim}980$ theorems: 30 files (83\%)
are signature-independent, 3 files (8\%) are compact-only (energy
positivity, Hilbert space positivity, mass gap), and 3 files (8\%)
...
($\alpha_1^\text{GUT} = \frac{5}{3} \alpha_Y$) are machine-verified in
...
\item \textbf{SO(14)} ($> M_1$): $3 \times \mathbf{64}_+$
  ...
  $b_\text{SO(14)} = -208/3 < 0$ confirms asymptotic freedom.\CO
\end{itemize}
...
corrections from the rich Higgs sector ($104 + 45 + 24 + 5 = 178$
scalar components), two-loop corrections, or split multiplet
spectra\CO.  This degree of unification is comparable to the MSSM
prediction of exact unification~\cite{dimopoulos1981}, though our
model is non-supersymmetric.
...
10^{34.4}$ years~\cite{superkamiokande2020} by a factor of $3.5
\times 10^4$\CO.  The proton lifetime is safely above the
...
The 178 total scalar components ($104 + 45 + 24 + 5$) provide
...
\item \textbf{Machine verification}: the algebraic foundation has been
  formally checked by computer.
\item \textbf{Additional generation mechanisms}: the $\mathrm{SO}(4)$
  factor and $\Cl(14) \supset \Cl(8)$ chain offer algebraic approaches
  to three generations not available to $\mathrm{SO}(10)$.
\end{enumerate}
...
The decomposition $91 = 45 + 6 + 40$ is the key structural
fact\MV.  It shows that $\mathrm{SO}(14)$ is the minimal orthogonal
group containing both the $\mathrm{SO}(10)$ GUT gauge group and a
6-dimensional gravity sector $\mathrm{SO}(3,1) \cong \mathrm{SO}(4)$.
...
from representation theory alone.  The most promising direction for
$\mathrm{SO}(14)$ is the extension of Gresnigt's $\Cl(8)$ $S_3$
symmetry mechanism~\cite{gresnigt2024,gresnigt2026} to the
64-spinor of $\Cl(14)$.  This is a concrete mathematical problem:
determine whether the $S_3$ action on three complex octonion
subalgebras within $\Cl(8)$ extends to an $S_3$ action on the
$\mathbf{64}$ of $\Cl(14)$ that (a) generates three copies of the
$\mathbf{16}$ of $\mathrm{SO}(10)$, (b) preserves the gauge
structure, and (c) is compatible with the $\mathrm{SO}(4)$ gravity
sector.  If this computation succeeds, it would provide the first
algebraic derivation of three generations; if it fails, the
$\Cl(8)$ mechanism does not extend to $\mathrm{SO}(14)$.
...
Our machine-verified energy positivity results (3 files, 8\% of theorems)
...
machine-verified mass gap statement applies to the compact form and
assumes energy positivity.  The physically relevant non-compact
theory requires a modified mass gap formulation.  We make no claim
on the mass gap beyond stating it precisely and verifying the
...
\item \textbf{Error catching}: the symmetric traceless $\mathbf{104}$
  ...
  \mathrm{SO}(2n_2)$.
\item \textbf{Completeness}: all 91 generators, all 6 anomaly
  conditions, all Goldstone boson counts, all gauge boson masses are
  individually verified, not just stated.
...
\item \textbf{Signature audit}: classifying proofs by signature
  dependence (83\% independent, 8\% compact-only, 8\% needs-revision)
...
\item \textbf{Threshold corrections}: systematic computation of
  threshold corrections from the 178-scalar spectrum to determine
  whether the 0.88\% coupling miss can be naturally closed.
\item \textbf{$\Cl(8) \to \Cl(14)$ three-generation bridge}:
  the concrete computation extending Gresnigt's $S_3$ mechanism
  from $\Cl(8)$ to the 64-spinor of $\Cl(14)$.
...
We have presented a candidate $\mathrm{SO}(14)$ unified field theory whose
algebraic foundation---91 generators decomposing as $45 + 6 + 40$,
anomaly cancellation, symmetry-breaking chain to the Standard Model with
corrected Higgs representations, and matter content $3 \times \mathbf{64}$---has
been machine-verified in Lean~4 with zero \texttt{sorry} gaps across
37 proof files.  One-loop renormalization group analysis yields coupling
unification at $M_\text{GUT} = 10^{16.98}\GeV$ (0.88\% miss with
threshold corrections), a proton lifetime of $10^{38.9}$~years (safely above
experimental bounds), and asymptotic freedom at all scales.
...
(the Millennium Prize problem).  We have stated each problem honestly and
identified the most promising resolution strategies.

The methodological contribution---applying interactive theorem provers to
GUT physics for the first time---provides a new standard for algebraic
rigor in theoretical particle physics.  We advocate for machine verification
...
\bibitem{gresnigt2024}
N.~G.~Gresnigt \textit{et al.},
``Algebraic realisation of three fermion generations with
$S_3$ family and unbroken gauge symmetry from $\mathrm{Cl}(8)$,''
Eur.\ Phys.\ J.\ C \textbf{84}, 1124 (2024)
[arXiv:2407.01580].

\bibitem{gresnigt2026}
N.~G.~Gresnigt,
``Electroweak structure and three fermion generations in
Clifford algebra with $S_3$ family symmetry,''
arXiv:2601.07857 (2026).

\bibitem{furey2014}
C.~Furey,
``Generations: three prints, in colour,''
JHEP \textbf{10}, 046 (2014)
[arXiv:1405.4601].

\bibitem{gillard2019}
A.~B.~Gillard and N.~G.~Gresnigt,
``Three fermion generations with two unbroken gauge symmetries
from the complex sedenions,''
Eur.\ Phys.\ J.\ C \textbf{79}, 446 (2019).

\bibitem{altarelli2005}
G.~Altarelli and F.~Feruglio,
``Tri-bimaximal neutrino mixing, $A_4$ and the modular symmetry,''
Nucl.\ Phys.\ B \textbf{741}, 215 (2006)
[arXiv:hep-ph/0512103].
...
Comput.\ Phys.\ Commun.\ \textbf{308} (2025).

\bibitem{wieser2021clifford}
E.~Wieser and U.~Song,
``Formalizing geometric algebra in Lean,''
Adv.\ Appl.\ Clifford Algebras \textbf{31} (2021).
...
in preparation for Advances in Applied Clifford Algebras (2026).
...
for unified model building,''
arXiv:1511.08771 (2015).

% === Weinberg angle ===

\bibitem{georgi1974weinberg}
H.~Georgi, H.~R.~Quinn, and S.~Weinberg,
``Hierarchy of interactions in unified gauge theories,''
Phys.\ Rev.\ Lett.\ \textbf{33}, 451 (1974).

% === Octonionic approaches ===

\bibitem{stoica2018}
O.~C.~Stoica,
``Leptons, quarks, and gauge from the complex Clifford
algebra $\mathrm{Cl}_6$,''
Adv.\ Appl.\ Clifford Algebras \textbf{28}, 52 (2018).

% === Additional GUT references ===

\bibitem{mohapatra1986}
R.~N.~Mohapatra,
\textit{Unification and Supersymmetry}
(Springer, 1986).
...
in \textit{The Standard Model and Beyond}, 2nd ed.\
(CRC Press, 2017).
...
Conf.\ Proc.\ C \textbf{7902131}, 95 (1979).

\bibitem{gellmann1979}
M.~Gell-Mann, P.~Ramond, and R.~Slansky,
``Complex spinors and unified theories,''
Conf.\ Proc.\ C \textbf{790927}, 315 (1979).
...
Bull.\ Am.\ Math.\ Soc.\ \textbf{47}, 483 (2010)
[arXiv:0904.1556].
...
\bibitem{avigad2021}
J.~Avigad,
``Understanding, formal verification, and the philosophy
of mathematics,''
J.\ Indian Counc.\ Philos.\ Res.\ (2021).

% === Gauge-Higgs unification ===

\bibitem{abe2024}
H.~Abe, T.~Kobayashi, Y.~Kawamura, \textit{et al.},
``$\mathrm{SU}(14)$ grand gauge-Higgs unification,''
arXiv:2403.02731 (2024).
