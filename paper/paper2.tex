...
observation connects three seemingly disparate decomposition methods:
Fortescue's symmetrical components (electrical engineering, 1918),
the Cartan--Weyl root space decomposition (Lie theory, 1913--1925),
and the principal grading by Coxeter elements \cite{coxeter1934}.
We show that the first two are instances of character decomposition
on compact groups at different levels of generality: the Fortescue
matrix is the character table of $\ZZ_N$, and the Cartan--Weyl
decomposition is a Fortescue-type decomposition of the adjoint
representation. We further show that Clifford grade selection rules
in $\Cl(3,0)$ force products of $\so(3)$ generators to skip odd
grades, a structural constraint that implies block-tridiagonal form
for Hamiltonians built from such generators; the general case for
$\Cl(n,0)$ follows by standard arguments. All concrete
instances---including $\so(3)$ structure constants, Casimir
eigenvalues, Killing form negativity, grade projections in $\Cl(3,0)$,
and the complete Cartan--Weyl decomposition of $\su(3)$---are
machine-verified in Lean~4 with zero \texttt{sorry} gaps. We
delineate two additional structural analogies (the mass gap as a
spectral stop band, and the Coxeter grading as generalized Fortescue)
that share the same mathematical parent but require qualification
before being called provable identities.
...
The generalization has a concrete consequence:
...
\item Each pair $(E_\alpha, F_\alpha, [E_\alpha, F_\alpha])$ forms an
  ...
  $[E_3, F_3] = H_1 + H_2$.
\item All $28$ independent brackets are verified individually.
\end{itemize}
...
$c_2(\text{fund}) = (n-1)/2 = 13/2$, computed by the standard formula.
...
Clifford grade selection rules impose a structural constraint on
operators built from Lie algebra generators.
...
\textup{(Proved by standard mathematical argument; not machine-verified.)}
...
All concrete computations for $\Cl(3,0)$, $\so(3)$, and $\su(3)$ in
this paper are machine-verified in Lean~4 \cite{demoura2021lean4}
with mathlib \cite{mathlib}, compiled with zero \texttt{sorry} gaps.
The Casimir eigenvalue table for general $\so(n)$ (\Cref{tab:casimir})
and the block-tridiagonal structure theorem (\Cref{thm:block-tridiag})
are standard results proved by conventional mathematical argument. \Cref{tab:proofs} summarizes the
...
We have further shown that Clifford grade selection rules in $\Cl(3,0)$
force products of $\so(3)$ generators to skip odd grades, a constraint
verified in Lean~4 that implies block-tridiagonal form for Hamiltonians
built from such generators. The generalization to $\Cl(n,0)$ follows
by standard arguments (the even subalgebra is always closed under
multiplication).
