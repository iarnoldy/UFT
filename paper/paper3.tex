...
a machine-checked scaffold in Lean~4 with the mathlib library: 87 proof files
containing over 2,900 verified declarations (theorems, definitions, and lemmas) with zero \texttt{sorry} gaps.  The scaffold includes structural results---five certified Lie algebra homomorphisms composing into the chain $\su(5) \hookrightarrow \so(10) \hookrightarrow \so(14) \hookrightarrow \so(14)_M \hookrightarrow \so(16)$ (bracket preservation verified for each embedding), the Jacobi identity for nine Lie algebras including the 248-dimensional $E_8$, and anomaly trace identities---alongside dimensional consistency checks that verify prerequisites for representation-theoretic claims.  The $\mathrm{SO}(14)$
...
mathlib library~\cite{mathlib}.  Specifically, 86 Lean~4 proof files
containing over 2,900 verified declarations have been compiled with zero
\texttt{sorry} gaps (unproved assertions).  The scaffold includes genuine algebraic structure---five certified Lie algebra homomorphisms ($\su(5) \hookrightarrow \so(10) \hookrightarrow \so(14) \to \so(14)_M \hookrightarrow \so(16)$, plus $\so(4) \hookrightarrow \so(14)$), Jacobi identities for nine Lie algebras (including $E_8$ at 248 dimensions via sparse structure constant iteration with \texttt{native\_decide}), the antisymmetric trace identity $\Tr(A\{B,C\}) = 0$, and the $\so(14) \to \so(14)_M$ type bridge connecting flat algebraic types to mathlib's matrix infrastructure---as well as dimensional consistency checks (anomaly prerequisites, Goldstone counts, beta-coefficient arithmetic).  Both tiers are checked with the same deductive rigor as
...
87 proof files containing over 2,900 verified declarations.  Five certified Lie algebra homomorphisms compose into a chain from $\su(5)$ to $\so(16)$, including a type bridge connecting flat algebraic types to mathlib's matrix infrastructure; $E_8$ is independently verified as a Lie algebra via sparse Jacobi identity.  One-loop renormalization group analysis [CO] yields coupling
