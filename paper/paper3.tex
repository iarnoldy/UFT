...
Lean~4 theorem prover---87 proof files, over 2,900 verified declarations, zero
\texttt{sorry} gaps.
...
87\% of the algebraic results hold for all real forms of $\so(14,\mathbb{C})$.
...
complete generation of fermions~\cite{fritzsch1975}.  But $\mathrm{SO}(10)$
...
Lean~4~\cite{demoura2021lean4} with mathlib~\cite{mathlib}: 87 proof files,
...
Both tiers are verified with the same deductive rigor as the
Flyspeck project~\cite{hales2017formal} or the liquid tensor
experiment~\cite{commelin2022liquid}.
...
$\Tr(A\{B,C\}) = 0$ (antisymmetric) %[MV]
  & \texttt{anomaly\_trace} & \texttt{trace\_anticomm\_zero} & Anomaly trace identity \\
...
A signature audit of 46 of the 87 proof files~\cite{uft-github}
...
E$_8$~\cite{lisi2007} & 248 & $\mathbf{248}$ & --- & Partial$^*$ & Yes & claimed & not computed \\
...
Here is what we built.  87 Lean~4 proof files. Over 2,900 verified
